...
Revision: targeted cold-read repair 5
...
## 7. Machine model, Project Source and current-source alignment
...
Current-source and Authority selection reuse existing Joyflow mechanisms rather than introducing a new Currentness object, Source Manager or dependency graph. Default-branch/merged repository facts, candidate branch/PR state, derived structural navigation, exact execution-object binding and current-source replay keep their existing meanings. Derived navigation may seed source loading but cannot override exact current repository/source facts.
...
## 10. Dependency-closed implementation obligations and discovery boundary
...
Read-only dependency discovery is GitHub-first and may close directly from exact current Web-visible repository/PR/commit/file evidence. Local Codex discovery is conditional: it is required only for a material repository/local/runtime fact that remains unavailable or insufficient after current GitHub/source replay. A Phase-A dependency finding is Evidence only and creates no mutation authorization.

Phase A must identify a dependency-closed implementation surface at the level needed to authorize mutation: authoritative source, generated outputs, runtime validators, tests/fixtures/examples and package/currentness rebuild obligations must be distinguished. It does not need to predict the exact future bytes of a successor that has not yet been implemented. Exact generated byte drift, generated file-set changes, manifest/SHA256 values and validation results are established after authorized implementation by the existing deterministic generation, freeze and validation mechanisms; they may not be guessed or predeclared as current Evidence.

The exact hand-edited mutation set is therefore closed from current source dependencies before implementation authorization, while deterministic generated/currentness outputs are rebuilt and proven from the actual successor during Phase B. This design does not itself authorize either class of mutation.
...
A Phase-A read-only dependency closure may be completed by the Web Brain directly when exact current GitHub/source evidence is sufficient. It must preserve UNKNOWN rather than inventing a missing local/runtime fact, and only that residual fact is eligible for bounded read-only Codex discovery. Phase A never requires implementation of a hypothetical successor merely to learn its future generated bytes.
