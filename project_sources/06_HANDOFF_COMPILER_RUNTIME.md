...
Before repository mutation, the operational runtime validates the approved Projection against the supplied current repository. The Projection is a bounded self-contained execution snapshot, not a second task truth source: it separates exact physical identity from logical role binding, prefers references and digests over repeated formal copies, derives display/compact/Gate views mechanically, and obtains fixed Codex authority policy from runtime policy. User approval binds only the material execution authorization envelope; deterministic Approval View rendering is an audit witness, while compiler/model/build compatibility is verified separately and cannot redefine approved material semantics. The repository identity, approved baseline, typed GitHub path observations and any exact local Path Discovery Return are replayed. If the current HEAD differs from the approved baseline, or an approved path has no current repository anchor, Codex stops before mutation. This check verifies the approved object without removing Codex bounded implementation judgment.
