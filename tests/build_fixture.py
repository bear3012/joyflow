...
def _object_capture(expected_obj, subject):
    if expected_obj['object_type']=='REPOSITORY':
        observation={'repository_id':expected_obj['object_id'],'remote_url':f"https://github.com/{expected_obj['object_id']}.git",'commit_sha':expected_obj['ref_or_sha256'],'role':'APPROVED_INPUT'}
...
def codex_return(projection):
    ...
    pr=None; artifact=None
    if projection['delivery']['requires_pr']:
        base=projection['task_object_lifecycle']['approved_input_object']['base_commit']
        pr={'repository_id':'example/repo','base_branch':'main','working_branch':'joyflow/task','pr_url':'https://github.com/example/repo/pull/42','base_commit':base,'head_sha':'deadbeef42','touched_files':['runtime/joyflow_dual_layer.py'],'diff_evidence_ref':'EXEC_DIFF'}
        result_obj={'object_type':'REPOSITORY','source_mode':'REPOSITORY_REF','object_id':'example/repo','ref_or_sha256':pr['head_sha']}
        result_commit=raw_capture('CAP_EXEC_RESULT','REPOSITORY_COMMIT',f"git rev-parse {pr['head_sha']}^{{commit}}",0,pr['head_sha']+'\n','',result_obj,{'repository_id':'example/repo','remote_url':'https://github.com/example/repo.git','commit_sha':pr['head_sha'],'role':'EXECUTION_RESULT'},'EXECUTION_RESULT','REPOSITORY_HEAD')
        ...
        cap=raw_capture('CAP_EXEC_DIFF','REPOSITORY_DIFF',f"git diff --binary {base} {pr['head_sha']}",0,'diff bytes','',result_obj,{'base_ref':base,'head_ref':pr['head_sha'],'changed_paths':pr['touched_files'],'diff_sha256':'2'*64},'PR_HEAD',pr['head_sha']); captures.append(cap); evidence_rows.append(exec_evidence('EXEC_DIFF','PR_HEAD',pr['head_sha'],cap))
...
def make_blocked_return(projection, *, status='BRAIN_ROUTE_CONFLICT', observed_repository_ref=None, additional_paths=None, mutation_performed=False, with_pr=False):
    ...
    if with_pr:
        ...
        ret['pr_evidence']=None; ret['artifact_evidence']=None; ret['mutation_summary']={'mutation_performed':mutation_performed,'cleanup_status':'COMPLETED' if mutation_performed else 'NOT_REQUIRED','residual_changed_paths':[]}
...
def brain_review_capsule(previous, projection, ret, bundle, *, brain_verdict='PENDING_BRAIN_REVIEW'):
    ...
    elif ret['pr_evidence']:
        pr=ret['pr_evidence']; target={'target_type':'REPOSITORY_PR_HEAD','repository_id':pr['repository_id'],'pr_url':pr['pr_url'],'head_sha':pr['head_sha'],'actual_changed_paths':pr['touched_files'],'changed_paths_evidence_ref':pr['diff_evidence_ref']}
    ...
    payload={'project_id':projection['project_id'],'task_id':projection['task_id'],'round_id':projection['round_id'],'source_execution_status':ret['execution_status'],'source_projection_digest':projection['projection_digest'],'source_codex_return_digest':ret['return_digest'],'source_evidence_bundle_digest':bundle['evidence_bundle_digest'],'source_task_object_lifecycle_digest':projection['task_object_lifecycle']['lifecycle_digest'],'source_execution_lifecycle_result_digest':ret['execution_lifecycle_result']['transition_digest'],'source_codex_return_evidence_ref':'EXEC_RETURN_BIND','source_evidence_bundle_ref':'EXEC_BUNDLE_BIND','source_evidence_row_digests':c.source_evidence_row_digest_map(bundle),'validation_results':validation_results,'review_target':target,'structural_review':structural_review,'brain_review_verdict':brain_verdict,'brain_review_evidence_refs':brain_refs,'user_acceptance':'PENDING_USER_ACCEPTANCE','user_acceptance_evidence_refs':[],'acceptance_not_applicable_reason':None,'merge_candidate_freeze_digest':None,'merge_status':'NOT_AUTHORIZED','unresolved_followups':copy.deepcopy(ret['unresolved_items']) if ret['execution_status']=='BLOCKED' else [],
      ...
      'cumulative_review':{'relevant_prior_behaviors':[{'behavior_id':r['behavior_id'],'status':(({'PRESERVE_REQUIRED':'PRESERVED','CHANGE_AUTHORIZED':'CHANGED_WITH_RECLOSURE','SUPERSEDE_AUTHORIZED':'SUPERSEDED_WITH_RECLOSURE'}[r['expected_disposition']]) if ret['execution_status']!='BLOCKED' else 'BROKEN'),'evidence_refs':[prior_support_refs[r['behavior_id']]],'review_basis':'Review only the prior behavior explicitly relevant to the current PR and enforce its approved disposition; do not reopen unrelated project history.'} for r in projection['task_anchor']['planning_context']['relevant_prior_behaviors']], 'impact_comparison':{'expected_paths':copy.deepcopy(projection['current_source_context']['selected_paths']) if target.get('target_type')=='REPOSITORY_PR_HEAD' else [],'observed_paths':copy.deepcopy(ret['pr_evidence']['touched_files']) if ret.get('pr_evidence') else [],'unexpected_paths':[],'status':'WITHIN_EXPECTED'},'exit_condition_results':[{'condition':x,'status':'PENDING','evidence_refs':[],'review_basis':'Brain review must explicitly close this current change-unit exit condition.'} for x in projection['task_anchor']['planning_context']['exit_conditions']],'residuals':[{'issue':x,'material_risk':'Unresolved execution item remains outside the closed result.','reopen_trigger':'Reopen when the blocker is resolved or the affected goal is attempted again.'} for x in ret['unresolved_items']],'verdict':'PENDING_BRAIN_REVIEW' if brain_verdict=='PENDING_BRAIN_REVIEW' else brain_verdict},
