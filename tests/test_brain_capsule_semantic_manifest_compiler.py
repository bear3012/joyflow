...
class BrainCapsuleSemanticManifestCompilerTests(unittest.TestCase):
    ...
    def test_existing_frozen_pr_replay_manifest_preserves_review_coverage(self):
        ...
        derived = c.derived_current_source_view(projection)
        self.assertNotIn("current_source_context", projection)
        self.assertEqual(derived["review_coverage_paths"], paths)
        self.assertEqual(derived["mutation_paths"], [])
        ...
        self.assertEqual(projection["execution_object"]["physical_object"]["digest"], capsule["task_anchor"]["repository_anchor"]["frozen_head_sha"])
        self.assertEqual(projection["task_anchor"]["repository_anchor"]["baseline_commit"], capsule["task_anchor"]["repository_anchor"]["baseline_commit"])
