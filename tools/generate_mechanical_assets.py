...
def evidence_bundle_schema(model):
    ...
    capture={'type':'object','additionalProperties':False,'required':['capture_id','tool','capture_kind','command','exit_code','stdout','stderr','stdout_sha256','stderr_sha256','observed_object','observation','subject_type','subject_id','capture_sha256'],'properties':{
      'capture_id':{'type':'string','minLength':1},'tool':{'const':'joyflow-typed-execution-evidence-runner'},'capture_kind':{'enum':['REPOSITORY_HEAD','REPOSITORY_COMMIT','REPOSITORY_STATE','ARTIFACT_SHA256','SOURCE_MATERIAL_SET','REPOSITORY_FILE','REPOSITORY_DIFF','TEST_COMMAND']},'command':{'type':'string','minLength':1},'exit_code':{'type':'integer'},'stdout':{'type':'string'},'stderr':{'type':'string'},'stdout_sha256':HEX,'stderr_sha256':HEX,'observed_object':obj,'observation':{'type':'object'},'subject_type':{'type':'string','minLength':1},'subject_id':{'type':'string','minLength':1},'capture_sha256':HEX}}
