...
def capsule_schema(model):
    ...
    return {'$schema':'https://json-schema.org/draft/2020-12/schema','$id':'joyflow://fibered-task-capsule-v5','type':'object','additionalProperties':False,
      ...
      'properties':{
        ...
        'task_anchor':{'type':'object','additionalProperties':False,'required':['project_id','task_id','task_version','goal','desired_result','non_goals','planning_context','change_scope','repository_anchor','artifact_anchor','anchor_digest'],'properties':{
          ...
          'change_scope':{'enum':model['change_scopes']},'repository_anchor':{'type':['object','null']},'artifact_anchor':{'type':['object','null']},'anchor_digest':HEX}},
...
def projection_schema(model):
    ...
    execution_object={'type':'object','additionalProperties':False,'required':['object_type','source_mode','object_id','expected_ref_or_sha256','source_material_refs'],'properties':{
      'object_type':{'enum':['REPOSITORY','ARTIFACT']},'source_mode':{'enum':['REPOSITORY_REF','EXISTING_ARTIFACT','NEW_ARTIFACT']},
    ...
    task_object_lifecycle={'type':'object','additionalProperties':False,'required':['lifecycle_version','route_type','approved_input_object','discovery_object','approved_execution_boundary','expected_result_contract','lifecycle_digest'],'properties':{
      'lifecycle_version':{'const':1},'route_type':{'enum':['REPOSITORY_CHANGE','REPOSITORY_DISCOVERY','ARTIFACT_REPAIR','NEW_ARTIFACT']},
    ...
    return {'$schema':'https://json-schema.org/draft/2020-12/schema','$id':'joyflow://codex-handoff-projection-v8','type':'object','additionalProperties':False,'required':req,'properties':{
      ...
      'current_source_context':{'type':'object','additionalProperties':False,'required':['problem_reality','context_status','historical_retrieval_refs','selected_paths','impact_coverage','excluded_context','unresolved_questions','expansion_triggers','source_binding'],'properties':{
...
def codex_return_schema():
    ...
    obj={'type':'object','additionalProperties':False,'required':['object_type','source_mode','object_id','ref_or_sha256'],'properties':{'object_type':{'enum':['REPOSITORY','ARTIFACT']},'source_mode':{'enum':['REPOSITORY_REF','EXISTING_ARTIFACT','NEW_ARTIFACT']},'object_id':{'type':['string','null']},'ref_or_sha256':{'type':['string','null']}}}
    ...
    return {'$schema':'https://json-schema.org/draft/2020-12/schema','$id':'joyflow://codex-execution-return-v8','type':'object','additionalProperties':False,'required':['artifact_type','project_id','task_id','round_id','capsule_digest','projection_digest','evidence_bundle_digest','technical_preflight','execution_status','execution_lifecycle_result','machine_results','pr_evidence','artifact_evidence','blocker_evidence_refs','mutation_summary','unresolved_items','brain_review_status','user_acceptance_status','merge_status','return_digest'],'properties':{
      ...
      'pr_evidence':{'oneOf':[pr,{'type':'null'}]},'artifact_evidence':{'oneOf':[artifact,{'type':'null'}]},
...
def evidence_bundle_schema(model):
    obj={'type':'object','additionalProperties':False,'required':['object_type','source_mode','object_id','ref_or_sha256'],'properties':{'object_type':{'enum':['REPOSITORY','ARTIFACT']},'source_mode':{'enum':['REPOSITORY_REF','EXISTING_ARTIFACT','NEW_ARTIFACT']},'object_id':{'type':['string','null']},'ref_or_sha256':{'type':['string','null']}}}
    capture={'type':'object','additionalProperties':False,'required':['capture_id','tool','capture_kind','command','exit_code','stdout','stderr','stdout_sha256','stderr_sha256','observed_object','observation','subject_type','subject_id','capture_sha256'],'properties':{
      'capture_id':{'type':'string','minLength':1},'tool':{'const':'joyflow-typed-execution-evidence-runner'},'capture_kind':{'enum':['REPOSITORY_HEAD','REPOSITORY_COMMIT','ARTIFACT_SHA256','SOURCE_MATERIAL_SET','REPOSITORY_FILE','REPOSITORY_DIFF','TEST_COMMAND']},'command':{'type':'string','minLength':1},'exit_code':{'type':'integer'},'stdout':{'type':'string'},'stderr':{'type':'string'},'stdout_sha256':HEX,'stderr_sha256':HEX,'observed_object':obj,'observation':{'type':'object'},'subject_type':{'type':'string','minLength':1},'subject_id':{'type':'string','minLength':1},'capture_sha256':HEX}}
    evidence={'type':'object','additionalProperties':False,'required':['evidence_id','authority','kind','ref','claim','claim_digest','produced_by','subject_type','subject_id','raw_output_ref','raw_output_sha256'],'properties':{
      'evidence_id':{'type':'string','minLength':1},'authority':{'const':'EXECUTION_EVIDENCE'},'kind':{'enum':['REPOSITORY_HEAD_OBSERVATION','REPOSITORY_COMMIT_OBSERVATION','ARTIFACT_SHA256_OBSERVATION','SOURCE_MATERIAL_SET_OBSERVATION','REPOSITORY_FILE_SNAPSHOT','REPOSITORY_DIFF','TEST_RESULT']},'ref':{'type':'string','minLength':1},'claim':{'type':'string','minLength':1},'claim_digest':HEX,'produced_by':{'const':'TOOL'},'subject_type':{'type':'string','minLength':1},'subject_id':{'type':'string','minLength':1},'raw_output_ref':{'type':'string','minLength':1},'raw_output_sha256':HEX}}
...
def final_path_decision_schema():
    ...
    return {'$schema':'https://json-schema.org/draft/2020-12/schema','$id':'joyflow://final-path-decision-v3','type':'object','additionalProperties':False,
      ...
      'properties':{
        ...
        'baseline_commit':{'type':'string','minLength':1},'allowed_path_items':{'type':'array','minItems':1,'items':item},
